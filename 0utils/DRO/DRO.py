...
class DRO(nn.Module):
    ...
    def __init__(self, alpha=25, beta=2, margin=0.5, loss="binomial",
                 p_lambda=0.1, p_lambda_neg=0.1, K=5, p_choice=2,
                 plambda_eq=1,
                 **kwargs):
        ...
        print("DRO, p_lambda:", self.p_lambda, "p_lambda_neg:", self.p_lambda_neg, "loss", self.loss, 'p_choice',
              self.p_choice)
        ...
        if self.p_choice == 1:
            ...
            for i in range(n):
                ...
                if self.K > pos_pair_loss.size()[0]:
                    p_index = np.random.choice(pos_pair_loss.size()[0], pos_pair_loss.size()[0], replace=True,
                                               p=np_p_pos)
                    n_index = np.random.choice(neg_pair_loss.size()[0], neg_pair_loss.size()[0], replace=True,
                                               p=np_p_neg)
                else:
