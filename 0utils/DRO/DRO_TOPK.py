...
class DRO_TOPK(nn.Module):
    ...
    def __init__(self, alpha=50, margin=0.5, beta=2, K=50,
                 select_TOPK_all=1, loss="binomial", **kwargs):
    ...
    def forward(self, inputs, targets):
        ...
        pos_mask = torch.eq(targets.unsqueeze(0), targets.unsqueeze(0).t()).float() - torch.eye(n).cuda()
        neg_mask = torch.ones((n, n)).cuda() - pos_mask - torch.eye(n).cuda()

        # neg_mask = eyes_.eq(eyes_) - pos_mask  # negative pairs index mat
        # pos_mask = pos_mask - eyes_.eq(1)  # positive pairs index mat

        pos_sim = torch.masked_select(sim_mat, pos_mask.bool())
        neg_sim = torch.masked_select(sim_mat, neg_mask.bool())
        ...
        if self.select_TOPK_all == 1:
            ...
            if self.K * 4 > all_loss.size()[0]:
                ...
                warnings.warn(
                    "K larger than the total number of pairs in the batch. The loss is calculated using all the pairs by default.")
            else:
            ...
            if self.K * 2 <= pos_loss.size()[0] and self.K * 2 <= neg_loss.size()[0]:
                ...
                warnings.warn(
                    "K either larger than the number of positive pairs or larger than negative pairs. The loss is calculated using all the positive pairs and the same number amount of negative pairs which have the largest losses by default.")
