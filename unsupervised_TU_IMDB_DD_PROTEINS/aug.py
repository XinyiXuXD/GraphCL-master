...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, aug, transform=None, pre_transform=None,
        ...
        super(TUDataset_aug, self).__init__(root, transform, pre_transform,
                                            pre_filter)  # do pre_transform in self.process()
        ...
        if self.data.x is None:
            # these datasets' (MUTAG...) node labels are available
            if not (self.name == 'MUTAG' or self.name == 'PTC_MR' or self.name == 'DD' or self.name == 'PROTEINS' or
                    self.name == 'NCI1' or self.name == 'NCI109'):
        ...
        self.data.ind = torch.arange(self.data.y.shape[0])
        self.slices['ind'] = self.slices['y'].clone()
    ...
    def get(self, idx):
        ...
        return self.select_aug(self.aug, data)

    def select_aug(self, aug, data):
        if aug == 'dnodes':
...
#     except:
#         data = data
#     return Data(x=data.x, edge_index=edge_index, y=data.y, ind=data.ind)
