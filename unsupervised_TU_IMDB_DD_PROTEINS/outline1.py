...
def pre_data(args):
    ...
    # pre_transform
    pre_transform = None
    # if args.DS == 'COLLAB' or args.DS == 'REDDIT-BINARY' or args.DS == 'REDDIT-MULTI-5K' or args.DS == 'IMDB-BINARY':
    if args.pre_transform == 'true':
        feat_str = args.feat_str
    ...
    train_data, eval_data = TUDataset(path, name=args.DS, aug=args.aug, pre_transform=pre_transform).shuffle(), \
                            TUDataset(path, name=args.DS, aug=args.aug, pre_transform=pre_transform).shuffle()
...
def train(model, Cost_emb, Cost_div, train_data, optimizer, device, args):
    model.train()
    # Cost_div.clubs.eval()
    loss_all = 0

    train_data_1 = deepcopy(train_data)
    train_data.aug, train_data_1.aug = args.aug

    loader1 = DataLoader(train_data, args.batch_size, shuffle=False)
    loader2 = DataLoader(train_data_1, args.batch_size, shuffle=False)

    args.n_classes = len(loader1.dataset)
    args.num_train = len(loader1)

    for data, data_aug in zip(loader1, loader2):
        optimizer.zero_grad()
...
def eval(model, eval_data, args):
    model.eval()
    eval_loader = DataLoader(eval_data, args.batch_size, shuffle=False)
